...
export class ExtedzOpenlayerComponent implements OnInit {
  ...
  ngOnInit() {
    this.tiffImage = this.tiffImage ? this.tiffImage : {jobId:null,imageUrl:null,extent:[0,0,0,0]};
  ...
  setTool(event:any, toolType?: any) {  
    this.featureCoordinats = this.service.setTool(this.currentToolType,event.value.name,event.value.color);
